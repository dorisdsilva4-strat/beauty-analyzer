...
# Topics / keywords / ingredients to track sentiment for. Edit freely.
TOPICS = [
    "glass skin", "wedding makeup", "kajal", "tinted lip balm", "lip tint",
    "matte lipstick", "clear sunscreen", "sunscreen", "kojic acid",
    "saffron serum", "serum", "retinol", "niacinamide", "vitamin c",
    "moisturizer", "toner", "cleanser", "pigmentation", "acne", "spf",
...
def term_stats(items, terms, min_hits=1):
    rows = []
    for term in terms:
        tl = term.lower()
        hits = [a for a in items if tl in a["text"].lower()]
        if len(hits) >= min_hits:
            rows.append({"term": term, "volume": len(hits),
...
def chart_volume(rows, title, subtitle, xlabel, top_n=10):
    ...
    rows = rows[:top_n][::-1]
    if not rows:
        ax.text(0.5, 0.5, "Nothing to show yet", transform=ax.transAxes,
...
def _sentiment_tab(items, source_label, terms, noun, mention_word, key_prefix):
    ...
    rows = term_stats(items, terms)
    l, r = st.columns(2)
    with l:
        p = chart_volume(rows, "What's Being Talked About",
                         f"{source_label} · top {noun}s by coverage",
                         f"{mention_word} mentioning {noun}")
        st.image(p, use_container_width=True)
        _dl("⬇️ Download (1080×1080)", p, f"{key_prefix}_volume.png")
    with r:
        p = chart_sentiment(rows, "How People Feel",
                            f"{source_label} · avg sentiment per {noun}")
        st.image(p, use_container_width=True)
...
def main():
    ...
    with st.sidebar:
        ...
        group_by = st.radio("Group sentiment by", ["Topics", "Brands"], index=0,
                            help="Topics = ingredients / products / trends. "
                                 "Brands = companies.")
    ...
    terms = TOPICS if group_by == "Topics" else BRANDS
    noun = "topic" if group_by == "Topics" else "brand"
    ...
    with t_news:
        _sentiment_tab(articles, "Google News · India", terms, noun, "Articles", "news")
    ...
    with t_yt:
        if not yt_key:
            ...
            _sentiment_tab(yt_comments, "YouTube · India", terms, noun, "Comments", "youtube")
